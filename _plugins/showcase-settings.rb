...
# 5) 早期在 documents:pre_render 规范 _tabs showcase 文档的 permalink（替换 :title → slug，清理 URL 缓存）。
#
# 依赖：
...
module Jekyll
  class ShowcaseSettings < Jekyll::Generator
    ...
    def generate(site)
      ...
      # 读取分页参数
      pag = sc['pagination'] || {}
      per_page     = (pag && pag['per_page']) ? pag['per_page'].to_i : 12
      sort_reverse = (pag && pag.key?('sort_reverse')) ? !!pag['sort_reverse'] : true

      # 3a) 对 _tabs 等集合中的 showcase 文档：在同 URL 下生成接管页（真正的 Page）
      create_takeover_pages!(site, per_page, sort_reverse, debug)

      # 3b) 兜底：对现有 layout: showcase 的 Page 注入 pagination（若未注入）
      inject_pagination_for_pages!(site, per_page, sort_reverse, debug)
    end
    ...
    def create_takeover_pages!(site, per_page, sort_reverse, debug)
      site.collections.each do |label, coll|
        next if label.to_s == 'posts'
        coll.docs.each do |doc|
          layout = doc.data['layout'].to_s
          ...
          # 纠正 Tab 文档的 permalink（让导航用正确的小写路径），并清 URL 缓存
          ...
          # 先合并 Tab 文档的前言数据（保留 icon/order 等），后面再覆盖关键字段
          ...
          # —— 关键：模拟 tabs 页面环境，让主题像“原 tabs 页面”那样渲染大标题/页眉
          ...
          # 带上 Tab 的正文内容（通常是一小段说明文字）；index.md 以便 Markdown 转换
    ...
    def inject_pagination_for_pages!(site, per_page, sort_reverse, debug)
